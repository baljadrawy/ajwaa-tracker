...
#### Backend (`backend/`)
- Express 4 + Node.js 20 Alpine
- 7 ملفات routes: auth, users, tickets, services, sectors, attachments, export
- JWT authentication (24 ساعة) + roleCheck middleware
- PostgreSQL pool connection (pg module)
- رفع مرفقات بـ Multer (10MB max)
- تصدير CSV بترميز UTF-8 عربي
...
- 7 صفحات كاملة: Login, Dashboard, Tickets, NewTicket, TicketDetail, Services, Settings
- AuthContext (JWT في state، بدون localStorage)
- Axios instance مع interceptors
- Layout: Navbar + Sidebar + RTL بالكامل
- CSS Modules لكل مكون
- ألوان GACA: #323c73 (أساسي), #23d2ae → #2385c9 (تدرج)
- خط Noto Sans Arabic
- Nginx multi-stage Dockerfile مع SPA fallback
- شعار GACA في: navbar, صفحة الدخول

#### Docker Compose
- 3 حاويات: postgres:16-alpine, backend (Node 20), frontend (Nginx)
- Healthcheck على PostgreSQL
- Volume للبيانات + المرفقات
- متغيرات بيئية للاتصال + JWT

#### دليل التثبيت (`INSTALL_GUIDE.docx`)
- مستند Word احترافي بالعربي RTL
- 8 أقسام: المتطلبات، التحميل، التشغيل، الدخول، الإعداد، الأدوار، الأوامر، حل المشاكل
- جداول ملونة بألوان GACA + صناديق ملاحظات وتحذيرات
- أكواد بتنسيق مميز (خلفية داكنة + خط Consolas)

### الهوية البصرية المستخرجة
- الأزرق الداكن: #323c73 (263,041 بكسل في اللوقو — اللون الرئيسي)
- التدرج: #23d2ae (تركواز) → #2385c9 (أزرق)
- اللوقو نُسخ إلى: frontend/public/logo.png + icon.png

---

## 2026-04-13 — تأسيس المشروع

### القرارات
- اعتماد الهيكل النهائي للتذاكر والخدمات بعد مراجعة ملف الإكسل الأصلي
- اعتماد PostgreSQL بدل SQLite (تزامن متعدد المستخدمين)
- Docker Compose بـ 3 حاويات (frontend + backend + postgres)
- جميع حقول التذكرة إلزامية — لا حفظ بدون تعبئة كاملة

### التعديلات على الإكسل الأصلي
- حذف: إفادة فريق تحليل الأعمال (غير مستخدم)
- حذف: الدعم المطلوب من علم + الهيئة (دُمجا في التعليقات)
- حذف: أعمدة فارغة (Column1, Column2, منسق الخدمة3)
- تعديل: حالة البيئة من UAT/PROD إلى Operation Support / BA
- تعديل: التصنيف إلى تشغيلي / تحليلي / نقل البيانات
- تعديل: المسؤولية إلى الهيئة / شركة علم فقط
- تعديل: الحالة إلى جديدة → تحت الإجراء → مغلقة
- إضافة: رقم تذكرة تلقائي (T-0001)
- إضافة: سجل تعليقات + audit log + مرفقات
- إضافة: القطاع والإدارة كحقلين منفصلين في الخدمة
- إضافة: المرحلة (Phase) لتقسيم دفعات الإطلاق

### الأدوار
- مشرف: كل الصلاحيات + إضافة خدمات/قطاعات/مراحل
- منسق: تذاكر خدماته فقط
- مدير: قراءة + تصدير

### المخرجات
- عرض تقديمي للفريق (ajwaa_tracker_plan.pptx)
- CLAUDE.md + DEV_LOG.md

### الخطوة القادمة
- ~~بناء Prototype تفاعلي~~ ✅
- ~~تطوير التطبيق الكامل بـ Docker~~ ✅
- اختبار التشغيل الفعلي بـ Docker Compose
- استيراد البيانات من ملف الإكسل الأصلي
- إضافة Charts للداشبورد (recharts أو chart.js)
